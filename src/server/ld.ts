...
export const AUTHOR_LIST = {
  ...
  codingsoft: {
    ...
    name: 'CodingSoft',
    url: 'https://github.com/CodingSoft',
  },
};
...
export class Ld {
  ...
  genOrganization() {
    return {
      ...
      'description':
        ...
        'Agentes is a work-and-lifestyle space to find, build, and collaborate with agent teams that grow with you.',
  ...
  getAuthors(ids: string[] = []) {
    ...
    if (ids.length === 1 && ids[0] === 'codingsoft') return defaultAuthor;
    const personId = ids.find((id) => id !== 'codingsoft');
  ...
  genArticle({
    ...
    const baseInfo: any = {
      ...
      'inLanguage': locale,
      'keywords': tags?.join(' ') || 'CodingSoft Agentes',
